 ...
 * @property int $office_status
 * @property string $token_qvatel
 *
...
class Setting extends BaseModel
{
    ...
    public function rules()
    {
        return [
            ...
            [['description'], 'string'],
            [['token_qvatel'], 'string','max' => 255],
            [['email'], 'email'],
            [['save_api_logs','office_status'], 'integer'],
        ];
    }
    ...
    public function attributeLabels()
    {
        return [
            ...
            'office_status' => Yii::t("backend", "Estado de oficina"),
            'token_qvatel' => Yii::t("backend", "Token qvatel"),
        ];
    }
    ...
    public static function getValueOfFieldName($fieldname)
    {
        $model = self::find()->one();
    ...
    public static function getOfficeStatusWithTags() {
        $status = (int) self::getValueOfFieldName('office_status');
        if($status === 1) {
            $result = Html::a('<i class="fa fa-circle text-success"></i> Online', ['/setting/change-office-status']);
        } else {
            $result = Html::a('<i class="fa fa-circle text-danger"></i> Offline', ['/setting/change-office-status']);
        }
